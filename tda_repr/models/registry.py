...
def _pick_transformer_blocks_generic(model: nn.Module) -> List[str]:
	all_named = set(dict(model.named_modules()).keys())
	prefixes = ["transformer.layer", "model.layers", "encoder.layer", "layers"]
	best_prefix = ""
	best_ids: List[int] = []
	for p in prefixes:
		ids: List[int] = []
...
def _tv_weights_convnext(pretrained: bool):
	if not pretrained:
		return None
	return ConvNeXt_Tiny_Weights.DEFAULT


def _tv_weights_efficientnet_b0(pretrained: bool):
	if not pretrained:
		return None
...
def get_model_info(kind: str, device: Optional[torch.device] = None, pretrained: bool = False) -> ModelInfo:
	...
	if kind in ("convnext_tiny", "convnext"):
		model = tvm.convnext_tiny(weights=_tv_weights_convnext(pretrained))
		model.to(device)
	...
	if kind in ("efficientnet_b0", "efficientnet"):
		model = tvm.efficientnet_b0(weights=_tv_weights_efficientnet_b0(pretrained))
		model.to(device)
		layer_names = _filter_existing(model, _pick_features_blocks(model, max_per_stage=3))
