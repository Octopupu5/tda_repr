...
def evaluate_generation_bleu(
	...
	if pad_id is None:
		...
		if eos_id is None:
			raise ValueError("Tokenizer/model is missing pad_token_id and eos_token_id; cannot run generation benchmark.")
		pad_id = int(eos_id)
	else:
		pad_id = int(pad_id)

	# Avoid transformers spam: explicitly pass pad_token_id to generate() if not provided.
	if gen_kwargs.get("pad_token_id", None) is None:
		gen_kwargs["pad_token_id"] = int(pad_id)
	if gen_kwargs.get("eos_token_id", None) is None:
		eos_id2 = getattr(tokenizer, "eos_token_id", None)
		if isinstance(eos_id2, (tuple, list)):
			eos_id2 = eos_id2[0] if eos_id2 else None
		if eos_id2 is not None:
			gen_kwargs["eos_token_id"] = int(eos_id2)
