...
def _cleanup_memory(*, device: str = "") -> None:
	...
	if platform.system() == "Linux":
		try:
			...
			warned = bool(getattr(_cleanup_memory, "_trim_warned", False))
			if not warned:
				setattr(_cleanup_memory, "_trim_warned", True)
...
def _interactive_config_tui(args: argparse.Namespace) -> argparse.Namespace:
	...
	while True:
		...
		if step == 21:
			q1 = inquirer.select(
				message="Compute q=1 spectra to analyze connectivity/cycles?",
				choices=["Yes", "No"] + _back_choice(step),
...
def _rewrite_progress_figures(
	...
	is_generation_like = any(m in set(available_metrics) for m in ("ppl", "loss_assistant_only"))
	if is_generation_like:
		priority = ["ppl", "loss_assistant_only", "loss", "accuracy", "f1_macro", "precision_macro", "recall_macro"]
	else:
		priority = ["accuracy", "f1_macro", "loss", "precision_macro", "recall_macro", "ppl", "loss_assistant_only"]
...
def _interactive_pick_bench_metrics(current: Tuple[str, ...], *, nlp_objective: str = "") -> Tuple[str, ...]:
	obj = str(nlp_objective).lower().strip()
	if obj == "generation":
		available = ["loss_assistant_only", "ppl", "loss"]
	else:
...
def _interactive_pick_layers_tui(current_layers: List[str], all_modules: List[str], model: nn.Module) -> List[str]:
	...
	selected = inquirer.checkbox(
		...
		choices=choices,
		instruction="ArrowUp/Down navigate, Space select, Enter confirm",
	).execute()
	if not selected:
...
def main():
	...
	ap.add_argument("--runs_base", type=str, default="runs/exp", help="Base path prefix for run directories.")
	ap.add_argument(
		"--interactive",
	...
	def run_one(finetune_mode: str) -> str:
		...
		store = RunStore(str(args.runs_base), suffix=f"{args.task}_{args.dataset}_{args.model}_ft-{finetune_mode}", unique=True)
		store.write_meta(
			{
				"name": "exp",
				"task": args.task,
		...
		for epoch in range(int(args.epochs)):
			...
			if str(args.task).lower().strip() == "nlp" and nlp_objective == "generation":
				print(
					f"[Epoch {epoch}] val_loss_assistant_only={los} val_ppl={ppl} "
