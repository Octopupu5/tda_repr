...
def _pick_primary_metric(bench: dict) -> Tuple[str, str]:
	if isinstance(bench, dict):
		...
		if isinstance(bench.get("loss_assistant_only"), (int, float)):
			return "loss_assistant_only", "loss_assistant_only"
		if isinstance(bench.get("loss"), (int, float)):
			return "loss", "loss"
	return "unknown", "Metric"


def _pretty_metric_label(metric: str, i18n: I18N) -> str:
	return i18n.bench_metric_label(metric)
...
def _plot_one(
	...
	*,
	...
	ax_bot.set_xlabel(i18n.epoch(), fontweight="bold")
	if not bool(show_axis_labels):
		ax_bot.set_ylabel("")
	if bool(show_legends):
...
def main() -> None:
	...
	def auto_metric(recs: Sequence[dict], meta: dict, override: str) -> Tuple[str, str, str]:
		...
		if str(override).strip():
			...
			return bench_key, m, _pretty_metric_label(m, i18n=i18n)
