...
def _pretty_quality(metric: str, i18n: I18N) -> str:
	m = str(metric).lower().strip()
	if m == "f1_macro":
		return i18n.macro_f1()
	if m == "accuracy":
		return i18n.accuracy()
	if m == "ppl":
		return i18n.perplexity()
	if m == "loss_assistant_only":
		return i18n.loss_assistant_only()
	if m == "loss":
		return i18n.loss()
	return metric
...
def render_figure(
	*,
	...
	if f1_series:
		...
		ax[0].plot(x, y, color="#d62728", linewidth=3.0, marker="o", markersize=5.5, label=i18n.macro_f1())
	if acc_series:
	...
	if not f1_series and not acc_series:
		...
		ax[0].plot(x, y, color="#d62728", linewidth=3.0, marker="o", markersize=5.5, label=_pretty_quality(quality_metric, i18n=i18n))
	ax[0].set_ylabel(i18n.validation_pct() if (f1_series or acc_series) else i18n.validation_value())
	...
	labels = [i18n.depth_tag("early"), i18n.depth_tag("mid"), i18n.depth_tag("deep")]
	for j, desc in enumerate(descriptors):
		a = ax[j + 1]
		_beautify(a)
		for c, lay, tag in zip(colors, layers.as_list, labels):
			ser = desc_to_series[str(desc)].get(str(lay), [])
			if not ser:
				continue
			x = [e for e, _ in ser]
			y = [v for _, v in ser]
			lay_disp = str(lay).replace("distilbert.transformer.", "")
			a.plot(x, y, color=c, linewidth=2.7, marker="o", markersize=4.8, alpha=0.95, label=f"{tag}: {lay_disp}")
	...
	ax[2].set_xlabel(i18n.epoch(), fontweight="bold")
	ax[3].set_xlabel(i18n.epoch(), fontweight="bold")

	fig.subplots_adjust(left=0.08, right=0.98, top=0.98, bottom=0.10, wspace=0.34, hspace=0.22)
...
def main() -> None:
	...
	ap.add_argument("--quality_metric", type=str, default="")
