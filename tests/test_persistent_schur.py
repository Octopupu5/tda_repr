...
def _op_to_dense(op) -> np.ndarray:
	n = op.shape[0]
...
def _schur_from_dense(Delta: np.ndarray, idx_K: np.ndarray, idx_C: np.ndarray) -> np.ndarray:
	if idx_C.size == 0:
...
def _partition_q_simplices(L: SimplicialComplex, K: SimplicialComplex, q: int) -> tuple[np.ndarray, np.ndarray]:
	L_q = L.simplices(q)
...
def test_up_schur_matches_pinv_q1_two_triangles() -> None:
	...
	S_op = _op_to_dense(op)
	Delta_up_L = up_laplacian(L, q=1).toarray()
	idx_K, idx_C = _partition_q_simplices(L, K, q=1)
	S_pinv = _schur_from_dense(Delta_up_L, idx_K=idx_K, idx_C=idx_C)
